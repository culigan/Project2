            ...
            if (!error && response.statusCode == 200) {
                ...
                jserv.category(category, function (errorT, responseT, resultTitle) {
                    if (!errorT && responseT.statusCode == 200) {
                        response.render('question', ({ category: resultTitle.title, difficulty: data[0].value, answer: data[0].answer, question: data[0].question, type: "Jeopardy", all: JSON.parse(result.body)}));
                    }
                });
            }
            else
